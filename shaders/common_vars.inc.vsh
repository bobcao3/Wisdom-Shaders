...
void calcCommon() {
	...
	const vec3 suncolor_sunrise = vec3(0.7843, 0.6, 0.313) * 0.53;
	const vec3 suncolor_noon = vec3(1.192, 1.2235, 1.2156) * 1.2;
	const vec3 suncolor_sunset = vec3(0.7843, 0.439, 0.2745) * 0.5;
	...
	vec3 ambient_sunrise = vec3(0.443, 0.772, 0.886) * 0.12 * TimeSunrise;
	vec3 ambient_noon = vec3(0.176, 0.392, 1.0) * TimeNoon;
	vec3 ambient_sunset = vec3(0.372, 0.615, 0.847) * 0.13 * TimeSunset;
	...
	vec3 horizontColor_sunrise = vec3(2.1, 1.8, 1.0) * 0.13 * TimeSunrise;
	vec3 horizontColor_noon = vec3(2.1, 2.18, 2.16) * 0.27 * TimeNoon;
	vec3 horizontColor_sunset = vec3(2.1, 1.6, 0.8) * 0.13 * TimeSunset;
	...
	totalSkyLight = mix(vec3(0.2, 0.45, 1.0), vec3(0.25), rainStrength);
